...
namespace CodeAnalysis
{
  ...
  public class TypeTable  // holds detected Type information
  {
      public string type { get; set; }
      public string name { get; set; }
  ...
  public class Repository
  {
    ...
    List<TypeTable> typeTable_ = new List<TypeTable>();
    List<Relationship> relationshipTable_ = new List<Relationship>();
    List<PackageDependency> packageDeptable_ = new List<PackageDependency>();
    ...
    public List<TypeTable> typetable
    {
    ...
    public List<PackageDependency> packageDeptable
    {
        get { return packageDeptable_; }
    }
  }
  ...
  {      
      public static string isDefinedType(string token)
      {
          ...
          foreach (TypeTable t in typetable)
              if (t.name == token)
                  return t.type;
          return "NOTINTABLE";
      }
  ...
  public class DetectUsing : ARule
  {
      public override bool test(CSsemi.CSemiExp semi)
      {
          ...
          if (index > 0 && !DetectFunction.isSpecialToken(semi[index - 1]))
          {
              ...
              foreach(TypeTable t in r.typetable)
